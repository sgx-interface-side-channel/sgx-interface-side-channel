...
// How?
// 1.找到所有的branch
// 2(option).如果一个节点仅有一个后序节点，后序节点也只有这一个时，将这两个节点combine。-simplifycfg
// 3.如果一个节点有两个不同的前序节点，查询这两个节点是否有一个共同的支配节点，如果有，则认为存在一个可消除的branch
// 4.消除方法：a.选择先后顺序 b.复制变量 c.插入branch code
        ...
        bool runOnFunction(Function &F) override {
            ...
            for(inst_iterator iter = inst_begin(F); iter != inst_end(F); ++iter){
                ...
                if(isa<BranchInst>(*iter)){
                    ...
                    //判断是不是根据条件跳转的指令，如果是的话需要对两个分支进行处理
                    if(ins->isConditional()){
                        ...
                        //将true分支的所有指令插入到false分支中
                        for(Instruction &ins_1 : *Bb_true){
                            if(!isa<BranchInst>(ins_1)){
                                Instruction* new_inst = ins_1.clone();
                                //把clone出来的新的指令的指针存到new_true_inss中
                                new_true_ins.push_back(new_inst);
                                //把原先的所有指令的指针存到Bb_true_ins中
                                old_true_ins.push_back(&ins_1);
                                //clone的指令并不会修改其操作数，所以我们要将操作数修改为该block中声明的变量
                                int pos = 0;
                                // Call指令需要修改函数的所有参数，所有参数都要复制原来的，创建新的，防止影响原来的程序
                                ...
                                // Store指令的第一个指令用原来的，第二个指令如果不在这个block内，则需要创建新的
